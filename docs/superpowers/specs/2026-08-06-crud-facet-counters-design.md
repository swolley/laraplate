...
- **Tier 2** (open/high-cardinality facets): `CrudService::facetValues(ListRequestData, FacetQuery): FacetPage` — one SQL `GROUP BY` on a key column, with the facet's own pagination, value `search` and `sort` (count/key), a `distinctValues` count, and the key≠label two-step (`resolveFacetLabels` resolves display `fields` in bounded `whereIn` reads over the page's keys). Labels may be base-table columns **or** a single-hop `relation.column` whose relation is a `BelongsTo` keyed by `groupBy` (e.g. group by `license_id`, label `license.uuid`) — resolved from the related model without any join, keeping `GROUP BY` on one portable column. **Label search/sort** is supported too (`labelField` + `FacetSort::LabelAsc`/`LabelDesc`): rather than joining the related table into the grouped query (which would collide with the unqualified ACL/soft-delete scopes — the original reason it was deferred), search resolves the matching related keys and applies a `whereIn`, and sort uses a **correlated subquery** in `ORDER BY` that yields each group's label (functionally dependent on the key). No `JOIN`, portable across the supported DBs. Still deferred: multi-hop label paths.
