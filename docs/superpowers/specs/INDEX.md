...
- `2026-08-15-sao-phase-3a-driver-framework-foundation-design.md`: **Proposed** — SAO phase 3a driver framework foundation: open driver registry, per-capability contracts (`issues`/`vcs`/`logs`/`releases`), `Connection` with env-referenced credentials (F4: secrets never in DB), and a per-capability conformance suite proven offline by one in-memory reference driver. No concrete external driver (Redmine is 3b); deliberately built before Phase 2.
