...
- `id`, `company_id` (`BelongsToCompany`, ERP), `content_id` **NOT NULL, UNIQUE** FK → `contents` (one extender per content, seam C15), `is_published_in_shop` bool, `featured` bool, `release_date` nullable, `metadata` JSON (marketing badges / promo flags)
...
`ContentExtenderRegistry::register('ecommerce.product', Product::class)`. CMS stores and reads the
...
- **Attribute taxonomy.** A map of where each attribute lives — Content `Preset` dynamic field
  (descriptive/spec, indexed via content) vs variant `attributes` JSON (size/colour, indexed via
  `extension`) vs merchandising (`extension`) vs ERP `Item` field (not indexed). Settle before building
  product forms and facets, or the same attribute gets modelled twice.
- **Stock reservation / overselling.** Whether v1 reserves stock at cart/checkout (an ERP-side
  reservation triggered by Ecommerce) or only checks availability at checkout, accepting a race. The
  in-stock browse snapshot (E21) is not a reservation.
- **Active-vendor resolution.** How the single active vendor `Company` per storefront (E12) is
  determined per request — domain, config, or an ERP-style current-company binding.
- **Cart→order vs payment timing.** When the ERP order is created relative to payment: before →
  `ecommerce_payment_reconciliations.order_id` is present; after → the reconciliation keys on the
  checkout session until the order exists. Tied to the cart→order handoff.
- **`company_id` placement.** E11 names it on products/variants/carts, but §6 puts it only on
  `ecommerce_products`. Confirm variants derive it from the product (no column) and only
  products/carts carry it.
