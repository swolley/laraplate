...
- **Price/stock browse snapshot (relaxes "no ERP in index").** Browsing needs price sort + range facet
  and an in-stock filter, which the strict "no ERP data in index" rule forbids. Decide whether to index
  a **browse-only, non-authoritative** price snapshot (and in-stock flag) refreshed on change — exact
  price/stock still resolved live from ERP at cart/checkout — and whether that snapshot is public-list
  only or per customer-group (Magento indexes per group).
