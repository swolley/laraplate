...
| E2a | **`Product` transparently merges its `Content` onto its own root.** Reading a `Product`, the content's fields (title, body, i18n, gallery, ...) are accessible as if they were the product's own; the `content` relation is loaded and proxied under the hood (a merged view), and writes to those fields route to the content inside the transactional create/update path — following the transparent-merge pattern already used elsewhere in the app (the User/collaborators approach). The ERP **`Item` link is NOT merged**: it stays an explicit relation via the variant pivot. | A mandatory 1:1 editorial body reads and writes naturally as one object; merging the `Item` would be ambiguous because it is many (variants/bundle) and volatile (stock/price) — which SKU, which price? |
