...
| E10a | **Review photos/videos = add the project media concern (`HasMultimedia`, as `Content` uses) to CMS `Comment`** — a generic CMS capability, not Ecommerce-only. Media visibility is gated by comment moderation exactly like the body/rating (unapproved comment → media not public). Which forms expose the upload is **form-context** (CMS admin omits them, the Ecommerce review form shows them), not a model split. | Media reviews are standard in ecommerce and cost one trait on a model that any commenter benefits from; consistency with `Content`'s media stack and the moderation gate keep it clean. |
