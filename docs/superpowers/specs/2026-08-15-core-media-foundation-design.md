...
| M5 | **`HasMedia` and `MediaFileNamer` move to Core** (both generic — only Core/spatie/`Str` deps; `MediaFileNamer` only special-cases the `thumb-` prefix and falls back to spatie's default). The app-wide `file_namer` config therefore binds Core's `MediaFileNamer`. **`HasMultimedia` stays in CMS** — it hardcodes content collections (`cover/images/videos/audios/files`) and image/video conversions, which are a CMS content concern SAO would never use; it references Core's `HasMedia`/`Media`. | Core can't reference a CMS class from its global config, and the generic media lifecycle belongs with the foundation; only the content-media shape is CMS-specific. |
