...
| M5 | **`Media` model, `HasMedia` trait and `MediaFileNamer` move to Core** (all generic — only Core/spatie/`Str` deps); the app-wide `media_model`/`file_namer` config binds Core's classes. **`HasMultimedia` stays in CMS for now** — although technically generic (its only non-spatie reference is the media model, now Core's), it is opinionated (`cover/images/videos/audios/files` collections + image/video thumb conversions) and CMS is its only consumer today; it is repointed to Core's `HasMedia`/`Media`. It can be promoted to Core later if a second consumer needs it. | Core can only reference Core classes from its global config, and the generic media lifecycle belongs in the foundation; the opinionated multimedia shape stays with its sole consumer until a reason to share it appears. |
