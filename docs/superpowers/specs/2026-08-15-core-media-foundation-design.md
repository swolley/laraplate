...
| M5 | **All media code moves to Core**: the `Media` model, the `HasMedia` and `HasMultimedia` traits, and `MediaFileNamer`. None of them depends on any CMS domain model, enum or config — their only CMS reference is the media model, which becomes Core's — so nothing is technically CMS-specific. `HasMultimedia` is merely *opinionated* (predefined `cover/images/videos/audios/files` collections + image/video thumb conversions); keeping the whole media foundation in Core gives one ownership story. **CMS and SAO become pure consumers**: SAO uses `HasMedia`, CMS uses `HasMultimedia`, both from Core. | The global `file_namer`/`media_model` config can only reference Core classes, and consolidating media in the foundation is cleaner than splitting a technically-generic trait out on taste alone. |
