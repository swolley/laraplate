...
| M4a | **AI contributes to the media search document through a single, generic Core "searchable contributor" seam — built once in Core, not duplicated and not reusing CMS's content-specific registry.** Any Core searchable model exposes a contribution hook: `toSearchableArray()` serializes the model's own fields and then lets registered contributors, scoped by target model type, add their fields + mapping section. Core owns the index and base mapping; a contributor (AI, for `Media`) fills its section. The content extension seam's **search-contribution** part (C11/C14) is expected to sit on this same Core seam rather than a separate CMS-local mechanism; the content **ownership** machinery (upcast/hide/cascade, `ContentExtenderRegistry`, already partly implemented in CMS) stays in CMS and is untouched. | Media standalone + parent enrichment need AI-derived fields in the index, but Core cannot reference AI. "Add a section to a searchable document" is generic and needed by both media and content, so it lives in Core once (no duplicated rules to maintain). Ownership is content-specific and correctly stays in CMS. |
...
| M21 | **Analysis models are a config-driven registry per capability, not a hardcoded model — mirroring the embeddings registry.** Like `ai.features.embeddings` (a `models` map + an `active` key resolved by `EmbeddingModelRegistry`), media analysis declares, per **capability**, a set of supported models/backends with an `active` default that operators can switch or extend from config without code changes. Capabilities: **vision** (caption, idea, intent, image OCR) — candidates `claude-sonnet-5` (default), `claude-haiku-4-5`, `claude-opus-5`, OpenAI `gpt-4o`/`gpt-4o-mini`, a local Ollama vision model; **transcription** (audio/video) — candidates self-hosted Whisper (default) and an external Whisper API. Each entry declares its provider and capability (not every provider does vision). LLM calls reuse the existing neuron-ai `ProviderFactory` (Anthropic/OpenAI/Mistral/Ollama already wired). | Consistent with the embeddings registry the team already runs; keeps model/provider choice an operator decision (cost, quality, privacy) rather than a code constant, and folds the transcription vendor choice (in-house vs external) into the same registry with a privacy-preserving self-hosted default. |
...
- **The media search-document contributor seam — decided (M4a).** A single generic Core "searchable
  contributor" seam, used by media now and by the content search-contribution later; content ownership
...
- Analysis model registry — decided (M21): config-driven per capability, defaults `claude-sonnet-5`
  (vision) and self-hosted Whisper (transcription), reusing neuron-ai's `ProviderFactory`. Residual: the
  exact config shape (mirror `ai.features.embeddings.models`/`active`) and a `MediaAnalysisModelRegistry`
  twin of `EmbeddingModelRegistry`.
- **Confirm neuron-ai multimodal image input** (^3.2): whether the Anthropic provider can send images to
