...
- Edit: `Modules/CMS/app/Helpers/HasMultimedia.php` — `use Modules\Core\Helpers\HasMedia` and reference `Modules\Core\Models\Media` (HasMultimedia stays in CMS; the collections/conversions are CMS content concerns).
- Edit: `Modules/CMS/app/Models/Content.php` and any other consumer — resolve to Core's `HasMedia`/media model.
- Delete: `Modules/CMS/app/Models/Media.php`, `Modules/CMS/app/Helpers/HasMedia.php`, `Modules/CMS/app/Helpers/MediaFileNamer.php`, the CMS `create_media_table` migration, and `CMSTables::Media`.
