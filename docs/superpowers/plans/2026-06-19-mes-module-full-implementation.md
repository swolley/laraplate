...
**MODULO MES (Task 4-17) COMPLETO E VALIDATO.** Suite eseguita su PHP 8.5: **`php artisan test Modules/MES/tests` → 122 passed, 357 assertions, 0 failed** (include smoke test 9 resource Filament, pipeline sales-order, quality-plan auto-check, stock-shortage con consumo parziale e notifica).
...
- **Stock-shortage (consumo parziale)**: nuovo contratto `StockReader` + adapter `ErpStockReader` (legge `StockLevel`). Backflush e consumo manuale leggono la giacenza e consumano il **disponibile**: lo scarico `out` è per ciò che esiste (`quantity_consumed`), lo scoperto diventa `variance` negativa con `stock_shortage = true` + evento `MaterialShortageDetected`. Così la giacenza resta veritiera (scende al consumato reale, mai negativa — `recordOutbound` ERP rifiuta il negativo) e il flusso è non bloccante. Scelta concordata con l'utente su opzioni A/B/C → C (consumo parziale con variance). **Notifica**: listener queued `NotifyMaterialShortage` → `MaterialShortageNotification` ai destinatari per ruolo (config `mes.notifications.stock_shortage`, canale `database` di default).
