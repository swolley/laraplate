...
- **Restore / reconcile** — **[reference-only DONE 2026-08-06]** `restoreToRevision(targetSet, expectedSet, force)`: one reversible `Revert` version set reverting scalars (version ≤ target) and re-linking `reference` relations to the membership as-of-target without touching shared subjects; concurrency precondition (`currentRevision()` must equal `expectedSet`, else `AggregateRestoreConflictException`); a hard-deleted reference subject throws `MissingRestoreSubjectException` unless `force` skips it (`RestoreReport.skippedSubjects`). Owned recreate/revert and shared-subject impact preview remain deferred.
