...
```bash
cd Modules/SAO && composer validate --no-check-all --no-check-lock; cd ../..
```

Expected: `./composer.json is valid`. A warning about the missing `composer.lock` is expected and acceptable — `.gitignore` excludes it, matching ERP.
...
```bash
php artisan ai:index-rag-docs --help
```

Expected: the command exists and its help text renders. Do **not** run an actual index in this
phase — indexing requires an embeddings provider and an Elasticsearch instance, neither of which
phase 0 depends on. This step only confirms the corpus is in a location the indexer knows about.
...
## Task 6: Bring the pre-existing stubs up to standard

The provider and controller stubs generated by `nwidart` predate the tooling and violate it: no `declare(strict_types=1)`, unsorted imports, commented-out scaffolding. Until they are fixed, `composer test:lint` can never pass, so the phase-0 exit criterion is unreachable.

**Files:**
- Modify: `Modules/SAO/app/Providers/SAOServiceProvider.php`
- Modify: `Modules/SAO/app/Providers/RouteServiceProvider.php`
- Modify: `Modules/SAO/app/Providers/EventServiceProvider.php`
- Modify: `Modules/SAO/app/Http/Controllers/SAOController.php`
...
4. `composer validate --no-check-all --no-check-lock` passes inside `Modules/SAO`.
...
7. `Modules/SAO/app` contains no domain classes — only the four provider and controller stubs.
