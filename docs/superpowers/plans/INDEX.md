...
- `2026-08-15-sao-phase-3a-driver-framework-foundation.md`: **Approved, in progress** — SAO driver framework foundation (slice 3a): capability/ingest enums, driver base + per-capability contracts, open `DriverRegistry`, `Connection` model with encrypted-at-rest credentials + optional env override (F4), `ConnectionCredentialResolver`, and a per-capability conformance suite proven offline by an in-memory reference driver. No concrete external driver; built before Phase 2 by design. Spec: `docs/superpowers/specs/2026-08-15-sao-phase-3a-driver-framework-foundation-design.md`.
