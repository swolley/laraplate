...
- `2026-08-15-sao-phase-3b-issues-sync.md`: **In progress — 3b-core done** (SAO suite green, 190 passed); 3b-redmine and 3b-ui remaining — SAO bindings and issues synchronization (Phase 3b): `SyncDirection`/`NormalizedIssue`, `BindingContext` enriching capability calls, `Connection.config`, `ProjectBinding`, `TicketLink`, the network-free `internal` issues driver (reference, built first), an idempotent direction-aware `IssueSyncService`, then the Redmine driver behind its conformance gate and Filament connection/binding surfaces. Slices: 3b-core (offline), 3b-redmine (needs fixtures), 3b-ui. Spec: `docs/superpowers/specs/2026-08-15-sao-phase-3b-issues-sync-design.md`.
- `2026-08-15-core-media-foundation.md`: **Proposed, awaiting approval** — re-home media ownership (table `vend_media`, `Core\Models\Media`, app-wide `media_model` config) from CMS to Core with a guarded, data-safe migration; CMS becomes a consumer. Two tasks (Core, then CMS). Prerequisite for SAO 1b attachments. Spec: `docs/superpowers/specs/2026-08-15-core-media-foundation-design.md`.
- `2026-08-15-sao-phase-1b-ticket-enrichment.md`: **Proposed, awaiting approval** — SAO ticket enrichment (Phase 1b): due dates, project-scoped labels, watchers (record-only), attachments (spatie medialibrary), typed ticket relations, and advanced search + saved filters over the ACL-scoped visible query. Seven independent offline tasks. Spec: `docs/superpowers/specs/2026-08-15-sao-phase-1b-ticket-enrichment-design.md`.
