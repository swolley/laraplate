...
**Superseded by** `2026-09-12-es-multilingual-index-and-multimodel-embeddings.md` (Task 3), which changed `createIndex` to push the **full** translated mapping, not just the embedding field. That reverses this plan's embedding-only decision and **reintroduces the exact 400 this plan avoided**: the translator adds `meta` and `index: true` to the `nested`/`object` relation fields (tags/contributors/categories/locations) at `Modules/Core/app/Search/Translators/ElasticsearchTranslator.php:120-132`, and ES object/nested field types accept **neither** parameter. `stringifyFieldMeta` (`ElasticsearchEngine.php:793`) only stringifies the meta values; it does not remove `meta`/`index` from object/nested, so it does not prevent the failure. Verified 2026-09-16 against the live cluster: applying the full `Content` mapping returns `mapper_parsing_exception: Mapping definition for [tags] has unsupported parameters: [meta : {filterable=true, relation=tags}] [index : true]` (HTTP 400). The 2026-09-12 ES-gated tests skipped (no ES in the runner), so this was not caught. **Resolved 2026-09-17:** the sanitisation was placed at index-creation time (`ElasticsearchEngine::sanitizeMappingProperty`) rather than in the translator — the translator's `meta`/`index` output is deliberately kept because `ScoutSearchConstraintApplier` reads it back from `getSearchMapping()` for nested filtering. See the 2026-09-12 plan's "Pre-cutover blocker" section for the fix and its tests.
