...
todos:
  ...
  - id: erp-filament-resources
    content: "M4 — Filament **parziale storico, poi esteso da Spec 2B**: slice contabile + MVP commerciale + CRM + risorse M3 con righe DDT/GR/PO (2026-05-04) + colonne riepilogo PO + test `ERPFilamentCommercialResourcesTest` / `FoundationM3SchemaTablesTest` / dominio PO. Posting/chiusure contabili, parties, permessi granulari, report finanziari CSV e dashboard operative Sales Pipeline / Stock Valuation risultano implementati nei follow-up ERP."
    status: pending
  - id: erp-reporting-stub
    content: "M4 — Servizi report come query/jobs (no BI completa): `BalanceSheetService`, `IncomeStatementService`, `VatLedgerService` (registro IVA vendite/acquisti), `SalesPipelineService` (funnel opportunity -> won), `StockValuationService` (valore magazzino al costing method scelto). Output strutturato (JSON/array), pagine Filament per consumarli, export CSV finanziario implementato su bilancio di verifica, stato patrimoniale e conto economico. Dashboard operative Sales Pipeline e Stock Valuation includono filtri, KPI, empty state ed export CSV. PDF resta fuori scope."
    status: pending
  ...
  - id: einvoice-sdi-binding
    content: "M6.3 — **Completato.** Scope core: bind di `EInvoiceProvider`, provider stub deterministico, persistenza `EInvoiceSubmission`, azioni Filament minime per invio/refresh su fatture vendita postate e test del workflow. Phase 2C ha aggiunto schema dati FatturaPA/SDI, `FatturaPaAnagraphicMapper`, `FatturaPaXmlBuilder`, XSD ufficiale FPR12 vendorizzato, provider locale `fatturapa` con validazione XML e adapter `aruba` configurabile via HTTP. Non promette certificazione contro ambiente Aruba reale, polling/webhook SDI avanzato o conservazione sostitutiva. **Phase 2C attiva:** restano permessi admin estesi (prossimo 2C-04)."
    status: completed
...
- `business-filament-final`: scaffold Filament M3 completato; UX posting/unposting, parties e BI operativa v1 sono stati coperti dai follow-up ERP; API mobile resta fuori scope.
...
| `einvoice-sdi-binding` | M6.3 / Phase 2C | Stub `EInvoiceProvider`, submission e azioni UI completati; schema/readiness FatturaPA/SDI, mapper, XML/XSD FPR12 e adapter Aruba configurabile completati | IMPORTANTE per go-live IT |
...
3. Phase 2C: FatturaPA/SDI production readiness. Completati schema/readiness, mapping, XML/XSD e adapter Aruba configurabile; prossimo permessi.
...
Priorità: **Phase 2A** e **2B** sono completate; la priorità corrente è **2C (FatturaPA/SDI)** con prossimo task **2C-04 permessi admin estesi**, poi Phase 3+ (API azioni dominio, architettura).
...
- **E-invoicing**: M3.5/M6.3 hanno introdotto contratti `EInvoiceProvider`, DTO neutri e `e_invoice_submissions`. Decisione aggiornata Phase 2C: il modulo ERP include il percorso italiano FatturaPA/SDI come implementazione production-readiness, mantenendo il provider pluggable via config.
...
- **E-invoicing pluggable**: il contratto resta neutro (`EInvoiceProvider` + DTO), ma Phase 2C introduce il percorso italiano FatturaPA/SDI nel modulo ERP perché il go-live IT lo richiede. Altre giurisdizioni o provider alternativi restano estensioni/configurazioni, non fork del modulo.
...
E-invoicing nel modulo: `EInvoiceProvider` (interfaccia) + DTO neutri + tabella `e_invoice_submissions` (invoice_id, provider_code, external_id, status, last_payload_path, submitted_at, response_payload). Phase 2C aggiunge il percorso italiano FatturaPA/SDI: campi anagrafici, mapper, XML/XSD FPR12 e adapter Aruba configurabile sono presenti; permessi dedicati restano follow-up.
...
- **Reporting**: servizi come query/jobs (no BI completa) — `BalanceSheetService`, `IncomeStatementService`, `VatLedgerService`, `SalesPipelineService`, `StockValuationService`. Output strutturato JSON/array, pagine Filament per consumarli, export CSV finanziario e operativo implementato; PDF e BI completa restano fuori scope.
