...
todos:
  ...
  - id: einvoice-sdi-binding
    content: "M6.3 — **Completato.** Scope core: bind di `EInvoiceProvider`, provider stub deterministico, persistenza `EInvoiceSubmission`, azioni Filament minime per invio/refresh su fatture vendita postate e test del workflow. Phase 2C ha aggiunto schema dati FatturaPA/SDI, `FatturaPaAnagraphicMapper`, `FatturaPaXmlBuilder`, XSD ufficiale FPR12 vendorizzato e provider locale `fatturapa` con validazione XML. Non promette provider Aruba reale, polling/webhook SDI avanzato o conservazione sostitutiva. **Phase 2C attiva:** restano provider Aruba/configurabile (prossimo 2C-03) e permessi admin estesi."
    status: completed
...
| `einvoice-sdi-binding` | M6.3 / Phase 2C | Stub `EInvoiceProvider`, submission e azioni UI completati; schema/readiness FatturaPA/SDI, mapper e XML/XSD FPR12 completati; provider reale prossimo | IMPORTANTE per go-live IT |
...
3. Phase 2C: FatturaPA/SDI production readiness. Completati schema/readiness, mapping e XML/XSD; prossimo provider Aruba/configurabile, poi permessi.
...
Priorità: **Phase 2A** e **2B** sono completate; la priorità corrente è **2C (FatturaPA/SDI)** con prossimo task **2C-03 provider Aruba/configurabile**, poi permessi e Phase 3+ (API azioni dominio, architettura).
...
E-invoicing nel modulo: `EInvoiceProvider` (interfaccia) + DTO neutri + tabella `e_invoice_submissions` (invoice_id, provider_code, external_id, status, last_payload_path, submitted_at, response_payload). Phase 2C aggiunge il percorso italiano FatturaPA/SDI: campi anagrafici, mapper e XML/XSD FPR12 sono presenti; provider reale/configurabile e permessi dedicati restano follow-up.
