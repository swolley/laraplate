...
todos:
  ...
  - id: supplier-payment-runs-sepa-export
    content: "M5.1 follow-up / Spec 2B-13 — **Pending.** Distinte pagamenti fornitori e export tracciato bancario. Lo scadenzario registra debiti/crediti e allocazioni, ma non esegue pagamenti in banca. Aggiungere coordinate bancarie fornitore (`PartyBankAccount` con IBAN/BIC/default), `PaymentRun` approvabile, righe con snapshot beneficiario/importo/scadenza, export SEPA Credit Transfer `pain.001` per bonifici fornitori, checksum/metadati export e UI Filament minima. Non includere invio API banca, CBI, Ri.Ba, SDD o formati proprietari nel primo taglio."
    status: pending
