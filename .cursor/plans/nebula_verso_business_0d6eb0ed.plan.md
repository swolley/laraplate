...
| `einvoice-sdi-binding` | M6.3 / Phase 2C | Stub `EInvoiceProvider`, submission e azioni UI completati; schema/readiness FatturaPA/SDI e mapper completati; XML/XSD prossimo | IMPORTANTE per go-live IT |
...
3. Phase 2C: FatturaPA/SDI production readiness. Completati schema/readiness e mapping; prossimo XML/XSD, poi provider e permessi.
...
Priorità: **Phase 2A** e **2B** sono completate; la priorità corrente è **2C (FatturaPA/SDI)** con prossimo task **2C-01 XML/XSD**, poi provider/permessi e Phase 3+ (API azioni dominio, architettura).
...
E-invoicing nel modulo: `EInvoiceProvider` (interfaccia) + DTO neutri + tabella `e_invoice_submissions` (invoice_id, provider_code, external_id, status, last_payload_path, submitted_at, response_payload). Phase 2C aggiunge il percorso italiano FatturaPA/SDI: campi anagrafici, mapper, XML/XSD, provider configurabile e permessi dedicati.
